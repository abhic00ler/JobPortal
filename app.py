...
app.config['MYSQL_PASSWORD'] = "Mysql@2021!"
...
def manageprofile():
    if "user" in session:
        ...
        exist = cur.execute("SELECT * FROM profile WHERE profile_id = {}".format(user))
        ...
        if request.method == 'POST':
            ...
            if exist > 0:
                ...
                cur.execute("UPDATE profile SET college = (%s), department = (%s), education = (%s) WHERE profile_id = (%s)", (college, dept, education,user))
                mysql.connection.commit()
